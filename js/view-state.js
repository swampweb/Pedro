(() => {
  if (window.__pedroRejoinViewV2) return;
  window.__pedroRejoinViewV2 = true;
  const KEY='pedro.active.view.v1';
  function activeGame(){return document.querySelector('#game-view')?.classList.contains('active')}
  ...
  document.addEventListener('click',event=>{if(event.target.closest('[data-rejoin-table],[data-mod-live-table],[data-join-index],#start-game,#deal-cards'))setTimeout(()=>{if(activeGame())saveGame()},80);if(event.target.closest('#leave-table,#game-home'))saveLobby()},true);document.addEventListener('visibilitychange',()=>{if(document.hidden&&activeGame())saveGame();else if(!document.hidden)setTimeout(restore,60)});window.addEventListener('focus',()=>setTimeout(restore,60));window.addEventListener('pageshow',()=>setTimeout(restore,60));setTimeout(restore,150)
})();
